...
import { Upload, FileSpreadsheet, Save, ArrowRight, X, PenLine } from "lucide-react";
    ...
    <div className="rounded-2xl bg-gradient-hero text-white p-6 shadow-elevated flex flex-col lg:flex-row lg:items-center justify-between gap-4">
      <div>
        <div className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/5 px-3 py-1 text-xs mb-2 backdrop-blur-sm">
          <PenLine className="h-3 w-3" /> Input Manual
        </div>
        <h2 className="text-xl lg:text-2xl font-bold">Masukkan Data Komentar</h2>
        <p className="text-white/70 text-sm mt-1">
          Tulis atau tempel (paste) caption/komentar secara manual, atau upload file CSV/Excel untuk dianalisis sistem.
        </p>
      </div>
    </div>
      ...
      <div className="lg:col-span-3 rounded-xl border bg-card p-6 shadow-elegant">
        ...
        <form onSubmit={submitManual} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-2 md:col-span-2">
            <Label>Caption / Isi Komentar</Label>
            <Textarea rows={3} value={form.comment_text} onChange={e=>setForm({...form, comment_text: e.target.value})} placeholder="Tempel (paste) di sini caption atau komentar dari postingan yang ingin dianalisis..." />
          </div>
